...
const numGroups = 5; // Number of swarms and targets
...
function setup() {
  ...
  for (let i = 0; i < numGroups; i++) {
    // Each swarm group
    let swarm = [];
    for (let j = 0; j < 20; j++) { // 20 entities per swarm group
      swarm.push(new Entity(random(width), random(height), i));
    }
    swarms.push(swarm);

    // Corresponding target for each swarm group
    let angle = TWO_PI / numGroups * i;
    let x = circleOrigin.x + circleRadius * cos(angle);
    let y = circleOrigin.y + circleRadius * sin(angle);
    targets.push(new MovingTarget(x, y, circleRadius, circleSpeed, angle));
  }
}
...
class Entity {
  constructor(x, y, groupIndex) {
    ...
    this.maxSpeed = 10; // Maximum speed of the entity
