...
class Sprite {
  constructor(animation, x, y, speed) {
    ...
    this.index = 0;
    this.flipped = false;
    // Adjustable parameters for the rectangle boundingBox
    ...
    this.maxSpeed = 5;
    this.deceleration = 0.95; // Deceleration factor
  }
  ...
  animate() {
    ...
    // Move the sprite by the current velocity
    this.x += this.velocity;

    // Update the animation index if moving
    if (this.velocity != 0) {
  ...
  checkEdges() {
    // Calculate the actual edges of the rectangle
    let rectLeftEdge = this.x + this.w / 2 + this.rectOffsetX - this.rectWidth / 2;
    let rectRightEdge = this.x + this.w / 2 + this.rectOffsetX + this.rectWidth / 2;

    if (rectLeftEdge < 0) {
      this.x = -this.w / 2 - this.rectOffsetX + this.rectWidth / 2;
    } else if (rectRightEdge > width) {
      this.x = width - this.w / 2 - this.rectOffsetX - this.rectWidth / 2;
    }
  }
}
