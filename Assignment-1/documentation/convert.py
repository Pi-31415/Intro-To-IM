...
def parse_svg_to_p5js(svg_file_path, skip_points=0):
    ...
    for path in paths:
        ...
        for command in commands:
            # Convert command into int
            # Only write points based on the skip_points setting
            if point_counter == 0:
                if command['type'] == 'M' or command['type'] == 'L':
                    p5js_code.append(f'vertex({int(command["x"])}, {int(command["y"])});')
                elif command['type'] == 'Q':
                    p5js_code.append(f'quadraticVertex({int(command["cx"])}, {int(command["cy"])}, {int(command["x"])}, {int(command["y"])});')
                elif command['type'] == 'C':
                    p5js_code.append(f'bezierVertex({int(command["cx1"])}, {int(command["cy1"])}, {int(command["cx2"])}, {int(command["cy2"])}, {int(command["x"])}, {int(command["y"])});')
...
svg_file_path = r"/Users/pi/Desktop/pikamon.svg"

p5js_code = parse_svg_to_p5js(svg_file_path)

# Write to file
with open('pikamon.js', 'w') as file:
    file.write('\n'.join(p5js_code))
